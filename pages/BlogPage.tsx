...
const BlogPage: React.FC<Props> = ({ lang }) => {
  ...
  useEffect(() => {
    // Sync logic to get real-time updates from localStorage
    const loadArticles = () => {
      const saved = localStorage.getItem('huayu_articles');
      const allArticles = saved ? JSON.parse(saved) : ARTICLES;
      // Admin published content is auto-synced
      setArticles(allArticles.filter((a: Article) => a.status === 'published'));
    };

    loadArticles();
    // Re-check on focus or state changes
