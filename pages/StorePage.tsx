...
import { ShoppingCart, Star, Eye, ShieldCheck, X, ArrowRight, Check } from 'lucide-react';
...
const StorePage: React.FC<Props> = ({ lang }) => {
  ...
  useEffect(() => {
    // Load dynamic books
    const loadBooks = () => {
      const savedBooks = localStorage.getItem('huayu_books');
      setBooks(savedBooks ? JSON.parse(savedBooks) : BOOKS);
    };
    ...
    // Listen for storage changes (e.g. from admin tab)
            ...
            >
              <div className="w-full lg:w-1/3 aspect-[3/4] overflow-hidden rounded-[2rem] shadow-xl relative shrink-0">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                   <div className="bg-white/90 backdrop-blur-md px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest text-zinc-900">
              ...
              <div className="flex flex-col justify-between flex-grow">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                     <span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-[#C1121F]" /> {lang === 'EN' ? 'Hardcover' : 'হার্ডকভার'}</span>
                     <span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-[#C1121F]" /> {lang === 'EN' ? 'Fast Shipping' : 'দ্রুত শিপিং'}</span>
                  </div>
                </div>
            ...
            >
              ...
              <div className="flex-grow overflow-y-auto p-8 space-y-6 scroll-hide">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                    <div className="w-20 h-20 bg-zinc-100 dark:bg-zinc-800 rounded-[2rem] flex items-center justify-center mb-6">
                  ...
                  cart.map(item => (
                    <div key={item.id} className="flex gap-5 p-5 bg-zinc-50 dark:bg-zinc-800 rounded-[2rem] border border-zinc-100 dark:border-zinc-700 relative group">
                      <img src={item.image} className="w-16 h-20 object-cover rounded-xl shadow-md" alt={item.title[lang]} />
                      ...
                      <button 
                        ...
                        className="absolute top-4 right-4 p-1.5 bg-white dark:bg-zinc-900 text-zinc-400 hover:text-red-500 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 transition-all"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
              ...
              {cart.length > 0 && (
                <div className="p-10 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 space-y-8 rounded-t-[3rem]">
                  ...
                  <button 
                    onClick={() => { setIsCartOpen(false); navigate('/book-checkout'); }}
                    className="w-full py-6 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-[1.5rem] font-black uppercase tracking-widest text-[11px] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4"
                  >
